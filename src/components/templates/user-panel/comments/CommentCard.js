...
import ScoreStars from "@/components/modules/ScoreStars";
import {
  HiCursorArrowRays,
  HiOutlineClock,
...
export default function CommentCard({
  ...
  return (
    <Link href={`/user-panel/comments/answer/${_id}`}>
      <div className="user-panel-card">
        <div className="flex items-center gap-2">
          <HiCursorArrowRays className="icon-md text-catalan-400" />
          <h3 className="text-link md:text-regular text-catalan-800">دیدگاه ارسالی برای {productId.title}</h3>
        </div>
